...
spike_slab_block <- function(m,
  ...
  # intitalise objects
  theta <- list(location = matrix(NA, M, 1),
                scale = matrix(NA, M, 1))
  coefs <- tau <- delta <- list(location = matrix(NA, nrow = M, ncol = ncol(m$x)),
  ...
  for (k in 1:2) {
    ...
    delta[[k]][1, ] <- rbinom(n_params, 1, theta[[k]][1, ])
    tau[[k]][1, ] <- sample_tau(delta = delta[[k]][1, ], 
                                a_tau = hyper$a_tau[k], 
                                b_tau = hyper$b_tau[k], 
                                v_0 = v_0)
    m$tau[[k]] <- tau[[k]][1, ]
  ...
  for (mm in 2:M) {
    for (kk in 1:2) {
      ...
      theta[[kk]][mm, ] <- update_theta(a_theta = hyper$a_theta[kk], 
      ...
      ## update tau_lk
      tau[[kk]][mm, ] <- update_tau(a_tau = hyper$a_tau[kk], 
                                    b_tau = hyper$b_tau[kk], 
                                    v_0 = v_0, 
                                    beta = coefs[[kk]][mm - 1, ], 
                                    delta = delta[[kk]][mm, ])
