...
# just create the regression object
m <- lmls(y ~ s1 + s2 + s3 + s4 +s5 + n1 + n2 + n3 ,
          scale = ~ s1 + s2 + s5 + n1 + n2 + n3 - 1,
          data = test_data,
          light =FALSE)

# set the coefficients randomly so the algorithm actually converges
random_coefs <- sample(-20:20,9)
m <- set_coef(m,'location',random_coefs)
...
library(ggplot2)
barpl <- ggplot()
barplot(colMeans(delta$location), xlab = names(m$coefficients$location))
